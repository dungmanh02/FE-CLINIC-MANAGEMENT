...
// 🚀 ĐÃ IMPORT ĐẦY ĐỦ 3 HÀM API BỆNH ÁN (Bao gồm lấy đơn thuốc)
...
const PatientDashboard = () => {
  ...
  // STATE MODAL THUỐC VÀ CHI TIẾT LỊCH HẸN
  ...
  const handleLogout = (e) => {
    e.preventDefault(); localStorage.removeItem('token');
  ...
  const handleViewAppointmentDetails = async (id) => {
    ...
    } catch (error) { alert("Không tìm thấy thông tin chi tiết lịch khám!"); }
  };
  ...
  // 🚀 HÀM API: BỆNH NHÂN TỰ ĐỘNG GỌI LỊCH SỬ BỆNH ÁN CỦA CHÍNH MÌNH
  ...
  const handleCreateAppointment = async () => {
    try {
      ...
      alert("🎉 Đặt lịch hẹn thành công!"); setIsModalOpen(false); setFormData({}); fetchAppointmentsAndDepartments();
    } catch (error) { alert("Đặt lịch thất bại. Vui lòng kiểm tra lại ID Bác sĩ!"); }
  };

  const handleCancelAppointment = async (id) => {
    const reason = window.prompt("Nhập lý do bạn muốn hủy lịch hẹn:"); if (reason === null) return;
    try {
      await cancelAppointmentAPI(id, { reason: reason || "Người bệnh có việc bận cá nhân" });
      alert("🛑 Bạn đã hủy lịch hẹn thành công!"); fetchAppointmentsAndDepartments();
    } catch (error) { alert("Hủy lịch hẹn thất bại. Vui lòng thử lại!"); }
  };
  ...
  const handlePatientDeleteAppointment = async (id) => {
    if (window.confirm("🗑️ Bạn muốn xóa vĩnh viễn thẻ lịch hẹn này khỏi danh sách hiển thị không?")) {
      try {
        await deleteAppointmentAPI(id); alert("🗑️ Xóa lịch hẹn thành công!"); fetchAppointmentsAndDepartments();
      } catch (error) { alert("Xóa lịch hẹn thất bại. Vui lòng thử lại!"); }
    }
  };
  ...
  return (
    <div className="dashboard-container">
      ...
      {/* 🚀 5. MODAL XEM TOÀN BỘ SỔ LỊCH SỬ BỆNH ÁN CỦA MÌNH */}
      {isHistoryModalOpen && (
